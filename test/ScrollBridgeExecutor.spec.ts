...
let bridgeExecutor: ScrollBridgeExecutor;
...
const SCROLL_GAS_LIMIT = 15000000;
const SCROLL_VALUE = 0;

const encodeSimpleActionsSet = (
  bridgeExecutor: ScrollBridgeExecutor,
  target: string,
  fn: string,
  params: any[]
) => {
...
describe('ScrollBridgeExecutor', async function () {
  let snapId;
  ...
  before(async () => {
    ...
    // Mocking Scroll bridge messenger
    l1ScrollMessenger = await new MockL1ScrollMessenger__factory(user).deploy();
    l2ScrollMessenger = await new MockL2ScrollMessenger__factory(user).deploy();
    await l1ScrollMessenger.setL2Messenger(l2ScrollMessenger.address);
    await l2ScrollMessenger.setL1Messenger(l2ScrollMessenger.address);

    bridgeExecutor = await new ScrollBridgeExecutor__factory(user).deploy(
      l2ScrollMessenger.address,
  ...
  it('Check initial parameters', async () => {
    ...
    // Scroll Bridge Executor parameters
    expect(await bridgeExecutor.getEthereumGovernanceExecutor()).to.be.equal(
      ethereumGovernanceExecutor.address
    );
    expect(await bridgeExecutor.L2_SCROLL_MESSENGER()).to.be.equal(l2ScrollMessenger.address);
  });
  ...
  context('Ethereum Governance Executor queues an actions sets', () => {
    ...
    it('Tries to queue and actions set via Scroll L2 Messenger without being the Ethereum Governance Executor (revert expected)', async () => {
      ...
      await expect(
        l1ScrollMessenger
          .connect(user)
          .sendMessage(bridgeExecutor.address, SCROLL_VALUE, encodedData, SCROLL_GAS_LIMIT, {
            gasLimit: 12000000,
          })
    ...
    it('Queue and execute an actions set to set a message in Greeter', async () => {
      ...
      const tx = await l1ScrollMessenger
